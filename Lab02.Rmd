...
#the comma is so that you can throw in all rows
```
...
```
#we have to add a period to each of the above phrases, then shuffle them, and then concatenate them together
...
my_list
#you have to actually print it by calling "my_list"
```
...
```{r}
n = 14 * 3 * 2 * 3 * 6
treaments = letters[1:14]
variations = c("A", "B","C")
generations = c("Millenial", "GenX", "Boomer")
#X = data.frame(treatment = rep...,
X = data.frame(
  treatment = rep(treaments, each = 6 * 3 * 2 * 3),
  variation = rep(rep(variations, each = 18), times = 3),
  gender = rep(rep(c("m", "f"), each = 18), times = 3 * 3),
  generation = rep(rep(generatons , each = 6), times = 2  * 3 * 3),
  name = NA,
  y = NA
)
X
...
```{r}
expect_equal(as.numeric(my_reverse(v)), rev(v))
...
summary(iris)
It seems that the mean sepal length is 5.843, the sepal width is 3.057 and the petal length is 3.758 and petal width is 1.199. The distribution is largest for the petal length as opposed to the other components measurements.
```
...
tail(iris_sub)
summary(iris_sub)
#I got this code from partner in class 
```

Now create a vector `y` that is length the number of remaining rows in the data frame whose entries are 0 if "setosa" and 1 if "versicolor".

```{r}
if(data.frame_setosa = x)
  x = x + 0

if(data.frame_versicolor = x)
  x = x + 1
```
...
```{r}
threhold = sepal.length_y
```

What is the total number of errors this model makes?

```{r}
We can't be sure, but maybe 2 (estimation error and ignorance error.)
```
...
#' @param d           distance function (default is Euclidean distance)
#' @return            TO-DO: explain this argument
nn_algorithm_predict = function(Xinput, y_binary, Xtest){
  yhat = array(NA,nrow())
  
  for(istar in 1:nrow(Xtest)){
    yhat[istar] = which.min(rowSums(t(t(Xinput) - Xtest[istar,])^2))
