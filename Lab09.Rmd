...
This is good model because it illustrates what the various metrics are for this particular algorithm. 
...
```{r}
results_table$total_cost = cfn * results_tabl
```
...
```{r}
The RDC plots the variability of the values. 
```
...
```{r}
BS = (1/Xselect)sum(1, Xselect, (ytest - xtest)^2)
```
...
```{r}
It may look like what's expected because the Brier score is a particular score that utilizes in-sample and oos. 
```
